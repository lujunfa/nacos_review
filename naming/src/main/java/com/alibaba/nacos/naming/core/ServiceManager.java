...
public class ServiceManager implements RecordListener<Service> {
    ...
    public void onChange(String key, Service service) throws Exception {
        try {
            ...
            if (oldDom != null) {
                //如果这个service节点之前不为空，则直接替换掉对应数据就行
                oldDom.update(service);
            } else {
                //如果这个service节点本来就不存在，则需要放进缓存map中，并调用service初始化方法，开启健康检测
                putService(service);
                //初始化，开启健康检测
                service.init();

                //让该service监听数据一致性服务的变更，底层一致性协议是raft持久性一致和短暂性一致性。这里把数据同时保存到持久性一致服务和临时性一致服务
