    ...
    public void onReceiveChecksums(Map<String, String> checksumMap, String server) {
        ...
        try {
            ...
            for (Map.Entry<String, String> entry : checksumMap.entrySet()) {
                ...
                //如果本地没有或者本地为空，或者checkSum匹配不上则将该健加入到变更列表
            ...
            for (String key : dataStore.keys()) {
                //本地缓存的这个key不由发起同步的server负责，则过滤
                if (!server.equals(distroMapper.mapSrv(KeyBuilder.getServiceName(key)))) {
                    continue;
                }

                //本地缓存的这个key在发起同步的服务器上不存在，则本地的这个key标记为删除
            ...
            try {
                //针对需要修改的key，再次从发起同步的服务器上拉取对应的信息
            ...
            for (Map.Entry<String, Datum<Instances>> entry : datumMap.entrySet()) {
                ...
                if (!listeners.containsKey(entry.getKey())) {
                    ...
                    if (switchDomain.isDefaultInstanceEphemeral()) {
                        ...
                        //通知SERVICE_META_KEY_PREFIX监听器即ServiceManager ,service变更并注册service为监听器监听数据变化
